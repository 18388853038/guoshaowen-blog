...
async function main() {
  ...
  // 定期重试加载凭证（配置可能在启动后才写入）
  setInterval(function() {
    if (!bridgeStatus._connected) {
      var newCreds = loadCredentials();
      if (newCreds && newCreds.botId && newCreds.secret && (!creds || newCreds.botId !== creds.botId)) {
        log('INFO', 'New credentials detected, reconnecting...');
        creds = newCreds;
        bridgeStatus.account = creds.botId;
        connectWecomWS(creds);
      }
    }
  }, 30000);
...
async function connectWecomWS(creds) {
  try {
    ...
    client.on('message', async function(frame) {
      try {
        ...
        log('INFO', 'msgtype=' + body.msgtype + ' from=' + (body.from && body.from.userid) + ' chattype=' + body.chattype);
        ...
        if (body.msgtype === 'voice') {
          log('INFO', '语音消息: media_id=' + (body.media_id || body.voice?.media_id || '?'));
          try { var vp = require('./voice-pipeline'); } catch(e) { log('WARN', '语音模块: ' + e.message); }
          content = '🔊 [语音消息]';
        ...
        log('INFO', '文本: ' + content.substring(0, 200));
        ...
        log('INFO', '← 回复: ' + (replyText.substring(0, 200) || '(空)'));
    ...
    client.on('event', function(event) {
      log('INFO', '事件: eventtype=' + (event.event && event.event.eventtype) + ' from=' + (event.from && event.from.userid));
      bridgeStatus.lastEventAt = new Date().toISOString();
      bridgeStatus._lastConnectedAt = bridgeStatus.lastEventAt;
      updateHealthFile();
    });
    ...
    require('fs').watch(hotConfigPath, { persistent: false }, function(event) {
      if (event === 'change') {
        try {
          ...
          if (_new !== _lastConfig) {
            _lastConfig = _new;
            console.log('[热切换] wecom-bridge 检测到配置变化，退出由父进程看门狗重启...');
