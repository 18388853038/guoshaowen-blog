...
class DAGEngine {
  ...
  static computeTaskStatus(task, allTasks) {
    if (!task.dependsOn || task.dependsOn.length === 0) {
      // No dependencies: keep original status (unless it's 'blocked' from before)
  ...
  static detectCycle(tasks) {
    ...
    for (const t of tasks) {
      adj[t.id] = (t.dependsOn || []).filter(id => tasks.some(tt => tt.id === id));
    }
