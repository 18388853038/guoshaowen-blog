...
class SkillExecutor {
  ...
  async runPython(name, args) {
    ...
    return new Promise((resolve, reject) => {
      const proc = spawn('python', [pyPath, JSON.stringify(args)], {
        timeout: 15000,
      ...
      proc.on('close', code => {
        if (code === 0) {
          try { resolve(JSON.parse(out)); } catch(e) { resolve({ output: out }); }
        } else {
          resolve({ error: err, code });
        }
      });
      proc.on('error', e => reject(e));
      setTimeout(() => { proc.kill(); reject(new Error('超时')); }, 16000);
    });
  }
