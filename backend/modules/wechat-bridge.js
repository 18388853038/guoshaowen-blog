...
function startHealthServer() {
  var server = http.createServer(function(req, res) {
    function jsonReply(code, data) {
      var d = JSON.stringify(data || {});
      res.writeHead(code, { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(d) });
      res.end(d);
    }
    if (req.url === '/health' || req.url === '/') {
      jsonReply(200, bridgeStatus);
      ...
      req.on('end', function() {
        try {
          ...
          if (!content) { jsonReply(400, { error: 'missing content' }); return; }
          // 通过现有账号发消息
          if (gAccount) {
            // 使用现有的 sendMessage 函数发送（复用主流程的所有逻辑和参数）
            sendMessage(gAccount, to || '', content).then(function(r) {
              if (r === true) {
                jsonReply(200, { ok: true });
              } else {
                jsonReply(500, { ok: false, error: 'send failed', raw: r });
              }
            }).catch(function(e) {
              jsonReply(500, { ok: false, error: e.message });
            });
          } else {
            jsonReply(503, { ok: false, error: 'no active account' });
          }
        } catch(e) {
          jsonReply(400, { error: 'invalid json: ' + e.message });
        }
      });
    } else { jsonReply(404, { error: 'not found' }); }
  });
