...
class ErrorSink {
   ...
   */
  getStats() {
    var openCases = this.data.cases.filter(function(c) { return c.status === 'open'; }).length;
    var resolvedCases = this.data.cases.filter(function(c) { return c.status === 'resolved'; }).length;
    var totalCases = this.data.cases.length;
    ...
    return {
      ...
      byLevel: this.data.stats.byLevel,
      byTool: this.data.stats.byTool,
      recentCases: this.data.cases.slice(-5).map(function(c) {
  ...
  getTrendStats() {
    ...
    return {
      trend: trend,
      totalCases: this.cases ? this.cases.length : 0,
      byLevel: this.stats ? this.stats.byLevel : {},
      byTool: this.stats ? this.stats.byTool : {}
    };
  }
}
