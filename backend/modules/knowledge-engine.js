...
function searchKnowledge(query, opts = {}) {
  ...
  const results = entries.map(e => {
    ...
    if ((e.title || '').toLowerCase() === q) score += 10;
    else if ((e.title || '').toLowerCase().startsWith(q)) score += 8;
    else if ((e.title || '').toLowerCase().includes(q)) score += 5;
    ...
    // Category match
    if ((e.category || '').toLowerCase().includes(q)) score += 3;
...
function registerKnowledgeRoutes(registerRoute, parseBody, json) {
  ...
  registerRoute(['GET'], /^\/api\/kb\/entries$/, (req, res) => {
    ...
    json(res, { ok: true, total: entries.length, results });
  });
  ...
  registerRoute(['GET'], /^\/api\/kb\/search$/, (req, res) => {
    ...
    json(res, { ok: true, total: results.length, results });
  });
