...
var FILE_SYSTEM_TOOLS = [
  {
    id: 'read_file',
    name: 'read_file',
    description: '读取项目内指定文件的内容（UTF-8 文本）。收到代码修改任务时，先用此工具读取要修改的原文件。',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    id: 'write_file',
    name: 'write_file',
    description: '将内容保存到项目内的指定文件。这是你完成任务的必要工具——生成代码/方案后必须调用此工具保存文件，才算真正完成。路径相对于项目根目录，目录不存在会自动创建。',
    parameters: {
      type: 'object',
      properties: {
  ...
  switch (funcName) {
    ...
    case 'file_read':
      ...
      if (!targetPath.startsWith(BASE)) {
        return { success: false, message: '安全限制：不允许读取项目目录之外的文件' };
      }
    ...
    case 'file_write':
      targetPath = path.resolve(BASE, funcArgs.filepath || funcArgs.path || '');
      if (!targetPath.startsWith(BASE)) {
        return { success: false, message: '安全限制：不允许写入项目目录之外的文件' };
      }
      // ⭐ 路径隔离：检查 Agent 签名
    ...
    case 'file_list':
      ...
      if (!targetPath.startsWith(BASE)) {
        return { success: false, message: '安全限制：不允许列出项目目录之外的目录' };
      }
      if (!fs.existsSync(targetPath)) {
...
async function proactiveTaskPull(agentId) {
  ...
  try {
    ...
    if (claimedTask) {
      ...
      // 通知 agent 执行（简短消息）
      var execResult = await executeAgent(agentId, '【新任务】' + claimedTask.title + '。请立即执行。用 write_file 将成果写入文件，完成后用 complete_claimed_task 提交。', {
...
async function proactiveTaskPullAll() {
  try {
    ...
    return { total: 0, results: [], error: e.message };
  }
}
...
async function executeAgent(agentId, userMessage, options) {
  ...
  if (registry && agentInfo && agentInfo.skills) {
    availableTools = registry.getAgentTools(agentInfo.skills);
  }
  ...
  var _execTimeout = (options && options.timeout) || (options && options.taskId ? 120000 : 120000);

  for (var iter = 0; iter < MAX_TOOL_ITER; iter++) {
    // 总超时检查
    if (Date.now() - _execStartTime > _execTimeout) {
      if (!finalReply) finalReply = '[系统: 执行超时]';
      break;
    }
    var msgObj;
    if (availableTools.length > 0) {
      ...
      var text = await callAI(messages, options);
      finalReply = text;
      break;
    }
