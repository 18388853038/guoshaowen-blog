...
function findFfmpeg() {
  for (var i = 0; i < FFMPEG_CANDIDATES.length; i++) {
    try {
      execSync('"' + FFMPEG_CANDIDATES[i] + '" -version', { encoding: 'utf8', timeout: 3000, stdio: 'pipe', windowsHide: true });
      return FFMPEG_CANDIDATES[i];
    } catch(e) {}
  }
  // 尝试系统 PATH
  try {
    execSync('ffmpeg -version', { encoding: 'utf8', timeout: 3000, stdio: 'pipe', windowsHide: true });
    return 'ffmpeg';
  } catch(e) {}
  return null;
}
...
function convertToWav(inputPath, outputPath) {
  ...
  try {
    ...
    pyCode += ';print(o)';
    execSync('python -c "' + pyCode + '" "' + inputPath + '"', { encoding: 'utf8', timeout: 30000, windowsHide: true });
  ...
  try {
    ...
    execSync('python -c "' + pyCode2 + '" "' + inputPath + '"', { encoding: 'utf8', timeout: 60000, windowsHide: true });
...
async function transcribeLocal(audioPath, opts) {
  ...
  try {
    // 先测试 whisper 是否可用
    execSync('python -m whisper --help', { encoding: 'utf8', timeout: 5000, stdio: 'pipe', windowsHide: true });
  } catch(e) {
  ...
  try {
    ...
    var output = execSync(cmd, { encoding: 'utf8', timeout: 120000, maxBuffer: 10 * 1024 * 1024, windowsHide: true });
