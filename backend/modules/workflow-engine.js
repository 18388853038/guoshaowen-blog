...
function validateWorkflow(nodes, edges) {
  try {
    ...
    if (cycle) return { valid: false, error: '存在循环依赖: ' + cycle.join(' → ') };
...
function executeSteps(wf, steps, execution, data) {
  var nextStep = steps.find(function(s) {
    // Find first pending step whose dependencies are all complete
    if (execution.completedSteps.indexOf(s.id) >= 0) return false;
    if (execution.failedSteps.indexOf(s.id) >= 0) return false;
    
    if (!s.dependsOn) return true;
    var deps = Array.isArray(s.dependsOn) ? s.dependsOn : [s.dependsOn];
  ...
  var node = wf.nodes.find(function(n) { return n.id === nextStep.id; });
