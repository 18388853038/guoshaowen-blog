...
class AgentBoundary {
  ...
  checkRateLimit(agentId, toolName) {
    ...
    const result = harnessRules.getInstance().check(ctx);
  ...
  checkAndRecord(agentId, agentName, agentRole, toolName) {
    ...
    const engineResult = harnessRules.getInstance().check(ctx);
  ...
  getConfig() {
    return {
      ...
      ruleEngine: harnessRules.getInstance().getStats(),
      pendingRules: harnessRules.getInstance().getPendingRules(),
      stats: this.getStats()
    };
  }
