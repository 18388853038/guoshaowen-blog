/**
 * Telegram 桥接 - eCompany 独立子进程 v2
 * 消息双向转发：Telegram ↔ CEO
 * 使用 node-telegram-bot-api 接收/发送消息
 */
...
const PORT = process.env.TELEGRAM_HTTP_PORT || 28011;
const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN || '';
const ECOMPANY_HOST = '127.0.0.1';
const ECOMPANY_PORT = 8005;
const LOG_FILE = path.join(__dirname, '..', 'logs', 'telegram-bridge.log');
const STATUS_FILE = path.join(__dirname, '..', 'logs', 'telegram-bridge.status.json');
...
function forwardToECompany(message, from, chatId) {
  return httpPost(ECOMPANY_HOST, ECOMPANY_PORT, '/api/v4/channel/incoming', {
    message: message, from: from || chatId || '', channel: 'telegram'
  });
}
...
function startHealthServer() {
  ...
  }).listen(Math.max(PORT, 28011), function() {
...
async function main() {
  ...
  if (!TELEGRAM_BOT_TOKEN) {
    log('WARN', '暂无 Telegram Bot Token，等待配置');
  ...
  try {
    ...
    var bot = new TelegramBot(TELEGRAM_BOT_TOKEN, { polling: true });

    bot.on('message', async function(msg) {
      var text = msg.text || msg.caption || '';
      var chatId = msg.chat.id;
      var from = msg.from ? (msg.from.username || msg.from.id) : String(chatId);

      if (!text || msg.from && msg.from.is_bot) return;

      log('INFO', 'msg from ' + from + ': ' + (text.substring(0, 60) || '(empty)'));
      bridgeStatus.lastMessageAt = new Date().toISOString();
      bridgeStatus.messageCount++;
      bridgeStatus.lastEventAt = new Date().toISOString();

      try {
        var ecoResp = await forwardToECompany(text, String(from), String(chatId));
    ...
    log('ERROR', 'Telegram 初始化失败: ' + e.message);
    ...
    require('fs').watch(hotConfigPath, { persistent: false }, function(event) {
      if (event === 'change') {
        try {
          ...
          if (_new !== _lastConfig) {
            _lastConfig = _new;
            console.log('[热切换] telegram-bridge 检测到配置变化，退出由父进程看门狗重启...');
